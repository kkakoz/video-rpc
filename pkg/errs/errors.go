...
import (
	"errors"
	"fmt"
	httpstatus "github.com/kkakoz/video-rpc/pkg/errs/status"
	"runtime/debug"
...
func WithStack(code int, msg string) *Error {
	return New(code, string(debug.Stack()), msg)
}

func WithErrStack(code int, msg string, err error) *Error {
	return WithStack(code, msg).WithCause(err)
}
